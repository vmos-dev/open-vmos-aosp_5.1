...
{
    return writeFullyBody(m_buf, size);
}

int SocketStream::writeFullyBody(const void* buffer, size_t size)
{
    if (!valid()) return -1;
...
const unsigned char *SocketStream::readFully(void *buf, size_t len)
{
    ...
    while (m_remain<len)
    {
        inout_len=m_bufsize-m_remain;
        if (NULL == read(m_buf+m_remain, &inout_len))
        {
            return NULL;
        }
        m_remain += inout_len;
    ...
    if (m_remain)
    {
        memcpy(m_buf, m_buf+len, m_remain);
    }
