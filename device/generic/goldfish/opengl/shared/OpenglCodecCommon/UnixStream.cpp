...
int UnixStream::connect(const char * name, unsigned short port)
{
    m_sock = socket_local_client(name, ANDROID_SOCKET_NAMESPACE_FILESYSTEM, SOCK_STREAM);
    if (!valid()) 
    {
        ALOGE("Failed to connect to host errno=%d\n", errno);
        return -1;
    }
    return 0;
}
