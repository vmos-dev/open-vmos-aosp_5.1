...
public:
     ...
     *  NO_ERROR on success, or an appropriate error status.
     */
    virtual status_t getCurrentPreviewFrame(void* buffer, int stride);
    ...
    void setZoomVaule(int zoom) {
        m_nstatus[STATUS_ZOOMVALUE] = zoom;
    }
    int getZoomVaule() {
        return m_nstatus[STATUS_ZOOMVALUE];
    }

    void setFocusStatusVaule(int focus) {
        m_nstatus[STATUS_FOCUS] = focus;
    }

    // 获取聚焦成功标记
    int getFocusStatusVaule() {
        return m_nstatus[STATUS_FOCUS];
    }

    void setFocusVaule(const char* focus) {
        if (focus) {
            strcpy( &(mFocusVaule[4]), focus);
        }
    }

    char* getFocusVaule() {
        return &(mFocusVaule[4]);
    }

    // 0 不是auto模式   1 启动   2 已设置
    void setAutoFocusStatus(int focus) {
            *(int*)mFocusVaule = focus;
    }

    int getAutoFocusStatus() {
        return *(int*)mFocusVaule;
    }

    void setFocusAreas(const char* focus) {
        if (focus) {
            strcpy( mFocusAreas, focus);
        }
    }
    ...
    void setFlashMode(const char*flash) {
        strcpy(mFlash, flash);
    }
    char* getFlashMode() {
        return mFlash;
...
public:
    ...
    enum {
        STATUS_SHUT=0,        // 快门状态
        STATUS_FOCUS,      // 聚焦成功  // -1 true false
        STATUS_ZOOMVALUE,   // 放大
        STATUS_FACING,      // 前置后置
        STATUS_ROTATION,    // 重力旋转角度

        STATUS_MAX,         // end unused
    };
// 收到截图消息, 在停止过程中要设置状态,否则会有几率卡死
    int getstatus() {
        return m_nstatus[STATUS_SHUT];
    }
    void setstatus(int nm) {
        m_nstatus[STATUS_SHUT] = nm;
    }

    int getFacing() {
        return m_nstatus[STATUS_FACING];
    }
    void setFacing(int nm) {
        m_nstatus[STATUS_FACING] = nm;
    }

    int getRotation() {
        return m_nstatus[STATUS_ROTATION];
    }
    void setRotation(int nm) {
        m_nstatus[STATUS_ROTATION] = nm;
    }
...
//    int mZoomValue;
    // focus Finish status
//    int mFocusStatus;   // -1 true false
    char mFocusVaule[512];
    char mFocusAreas[512];
    char mFlash[512];
