...
static char* dl_TLS_SLOT_DLERROR = NULL;

static const char* __bionic_set_dlerror(char* new_value) {
  // char** dlerror_slot = &reinterpret_cast<char**>(__get_tls())[TLS_SLOT_DLERROR];
  char** dlerror_slot = &dl_TLS_SLOT_DLERROR;
...
#if 0//LIBCVM_pthread_create==0
  char* buffer = __get_thread()->dlerror_buffer;
#else
  static char dlerror_buffer[__BIONIC_DLERROR_BUFFER_SIZE];
...
void* libschappy_handle = NULL;
PFN_DLSYM_INTERCEPT dlsym_intercept = NULL;
...
void* dlsym(void* handle, const char* symbol) {
  ...
  if (sym != nullptr) {
    ...
    if ((bind == STB_GLOBAL || bind == STB_WEAK) && sym->st_shndx != 0) 
    {
        ...
        void* ret_address = reinterpret_cast<void*>(found->resolve_symbol_address(sym));
        // __libc_fatal_no_abort("[dlsym_intercept]:dlsym_intercept = %X, ret_address = %X, symbol = %s", dlsym_intercept, ret_address, symbol);
        if(dlsym_intercept)
        {
          ret_address = dlsym_intercept((void*)found, symbol, ret_address, NULL);
        }
        // __libc_fatal_no_abort("[dlsym_intercept]:dlsym_intercept = %X, ret_address = %X, symbol = %s", dlsym_intercept, ret_address, symbol);
