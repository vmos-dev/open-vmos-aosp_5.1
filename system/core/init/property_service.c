...
{
    void *data;
    int fd = open(PROP_FILENAME, O_RDWR | O_NOFOLLOW);  // 读写打开
    if (fd < 0)
        return -1;
...
int property_set(const char *name, const char *value)
{
    ...
    if(pi != 0) {
        ERROR("moe_opensrc: property_set 1 : %d [%s,%s] = %d \n", __LINE__, name, value, ret);
...
void property_load_boot_defaults(void)
{
    ...
    load_properties_from_file("/system.prop", NULL);
}
