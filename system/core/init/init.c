...
//        { "ro.boot.serialno", "ro.serialno", "", },
        ...
        { "ro.boot.hardware",   "ro.hardware",   "goldfish", }, //## add
...
int main(int argc, char **argv)
{
    ...
    while(true)
    {
        if (access("/dev/socket/socket/rfopengles22501", F_OK) == 0)
        {
            break;
        }
        usleep(10);
    ...
    property_set("rf.maximum.x", argv[1]);
    property_set("rf.maximum.y", argv[2]);
    property_set("ro.sf.lcd_density", argv[3]);
    property_set("ro.host.hardware.gralloc", argv[7]);
    property_set("rf.force.load", argv[8]);
    ...
    char sin[512]={0};
    snprintf(sin, 512, "/dev/socket/socket/rfopengles%d", 22501);
    while(true)
    {
        if (access(sin, F_OK) == 0)
        {
//            sleep(3);
            break;
        }
